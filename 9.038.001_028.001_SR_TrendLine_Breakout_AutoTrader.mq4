//+------------------------------------------------------------------+
//|           9.038.001_028.001_SR_TrendLine_Breakout_AutoTrader.mq4 |
...
void OpenPosition(bool SHORTLONG, double LOTS, double STOPLOSS, double TAKEPROFIT, int SLIPPAGE, int MAGICNUMBER)
{
   if(SHORTLONG)
   {
      ...
      OrderSend(Symbol(), OP_SELL, LOTS, Bid, SLIPPAGE, STOPLOSS, TAKEPROFIT, "SELL - Support breakout", MAGICNUMBER, 0, Red);
   }
   else
   {
      if(STOPLOSS > 0)
      ...
      OrderSend(Symbol(), OP_BUY, LOTS, Ask, SLIPPAGE, STOPLOSS, TAKEPROFIT, "BUY - Resistance breakout", MAGICNUMBER, 0, Blue);
   }
...
double Strategy(int STRATEGY, int COMMAND)
{
   switch(STRATEGY)
   {
// BREAKOUT: Trendlines draw to chart are SR levels - Green - possible breakout touching drawed trendiles
      case 29:
      {
         return(Strategy_029(COMMAND));
      }
   }
...
//------------------------------------------------------------------//------------------------------------------------------------------
double Strategy_029(int COMMAND)
{
...
//            break;
         ...
         for(i = 0; i < ObjectsTotal(); i++)
         {
            ...
            {
               // RESISTANCE BREAKOUT - If trendline has Red color - we are awaiting breakout
               if(ObjectGet(ObjectName(i), OBJPROP_COLOR) == Red)
               {
                  if(Ask > ObjectGetValueByShift(ObjectName(i), 0))
                  {
                     result = 1;
                     Print("BUY - Resistance breakout");
                     SendMail("BUY - Resistance breakout", "BUY - Resistance breakout");
                  }
                  break;
               }
            }
...
//            break;
         ...
         for(i = 0; i < ObjectsTotal(); i++)
         {
            ...
            if(ObjectType(ObjectName(i)) == OBJ_TREND)
            {
               // SUPPORT BREAKOUT - If trendline has Green color - we are awaiting breakout
               if(ObjectGet(ObjectName(i), OBJPROP_COLOR) == Green)
               {
                  if(Bid < ObjectGetValueByShift(ObjectName(i), 0))
                  {
                     result = 1;
                     Print("SELL - Support breakout");
                     SendMail("SELL - Support breakout", "SELL - Support breakout");
                  }
                  break;
               }
            }
