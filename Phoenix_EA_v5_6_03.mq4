  ...
  if(ChangeStopLoss==true)
   ...
   for(int y = total - 1; y >= 0; y --)
      ...
      if(OrderMagicNumber() == MAGICMA02 || OrderMagicNumber() == MAGICMA03)
         ...
         if(OrderType()==OP_BUY)
            {
            if(OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice(),OrderTakeProfit(),0,GreenYellow)==false)
            {
            err = GetLastError();
            Print("OrderSelect( ", y, ", SELECT_BY_POS ) - Error #",err );
            continue;
      ...
      for(int y = total - 1; y >= 0; y --)
         ...
         if(OrderMagicNumber() == MAGICMA03)
            ...
            if(OrderType()==OP_BUY)
               ...
               if(OrderModify(OrderTicket(),OrderOpenPrice(),NewSLTrade3B,OrderTakeProfit(),0,GreenYellow)==false)
                  {
                  err = GetLastError();
                  Print("OrderSelect( ", y, ", SELECT_BY_POS ) - Error #",err );
                  continue;
            ...
            if(OrderType()==OP_SELL)
               ...
               if(OrderModify(OrderTicket(),OrderOpenPrice(),NewSLTrade3S,OrderTakeProfit(),0,GreenYellow)==false)
                  {
                  err = GetLastError();
                  Print("OrderSelect( ", y, ", SELECT_BY_POS ) - Error #",err );
                  continue;
