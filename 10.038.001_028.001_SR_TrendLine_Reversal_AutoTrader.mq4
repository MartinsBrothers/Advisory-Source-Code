//+------------------------------------------------------------------+
//|          10.038.001_028.001_SR_TrendLine_Reversal_AutoTrader.mq4 |
...
extern string SUPPORT = "Support trend line have to be GREEN color";
extern string RESISTANCE = "Resistance trend line have to be RED color";
...
double Strategy(int STRATEGY, int COMMAND)
{
   switch(STRATEGY)
   {
// REVERSAL: Trendlines draw to chart are SR levels - Red - possible reversal touching drawed trendlines
      case 28:
      {
...
//            break;
         ...
         // iterate all ObjectsDeleteAll
         for(i = 0; i < ObjectsTotal(); i++)
         {
            // get only trendlines
            if(ObjectType(ObjectName(i)) == OBJ_TREND)
            {
               // If trendline has Green color - we are awaiting reversal - SUPPORT
               if(ObjectGet(ObjectName(i), OBJPROP_COLOR) == Green)
               {
                  if(Ask < ObjectGetValueByShift(ObjectName(i), 0))
                  {
                     result = 1;
                     Print("BUY - Support reversal: ", "Ask: ", Ask, " Trenline name: ", ObjectName(i), " Trendline value: ", ObjectGetValueByShift(ObjectName(i), 0));
                     //gSendMail ("", "radovan.rybar@rwe.com" , "BUY - Support reversal" , "BUY - Support reversal" , "" , "");
//                     SendMail("BUY - Support reversal", "BUY - Support reversal");
                  }
                  break;
               }
            }
...
//            break;
         ...
         for(i = 0; i < ObjectsTotal(); i++)
         {
            ...
            if(ObjectType(ObjectName(i)) == OBJ_TREND)
            {
               // If trendline has Red color - we are awaiting reversal - RESISTANCE
               if(ObjectGet(ObjectName(i), OBJPROP_COLOR) == Red)
               {
                  if(Bid > ObjectGetValueByShift(ObjectName(i), 0))
                  {
                     result = 1;
                     Print("SELL - Resistance reversal: ", "Bid: ", Bid, " Trenline name: ", ObjectName(i), " Trendline value: ", ObjectGetValueByShift(ObjectName(i), 0));
                     //gSendMail ("", "radovan.rybar@rwe.com" , "SELL - Resistance reversal" , "SELL - Resistance reversal" , "" , "");
//                     SendMail("SELL - Resistance reversal", "SELL - Resistance reversal");
                  }
                  break;
               }
            }
