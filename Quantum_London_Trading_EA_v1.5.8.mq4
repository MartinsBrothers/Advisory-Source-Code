...
double digits;
int openorders,mt1,mt2,mt3,thewait,cycles;
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
int init()
         ...
         if(((iTime(Symbol(),1,0)>=StrToTime(open1) && iTime(Symbol(),1,0)<StrToTime(close1)) || (iTime(Symbol(),1,0)>=StrToTime(open2) && iTime(Symbol(),1,0)<StrToTime(close2)) || (iTime(Symbol(),1,0)>=StrToTime(open3) && iTime(Symbol(),1,0)<StrToTime(close3)) || (fulltime==true)) && curtime>prevtime)
           {
            ...
            if(((iCustom(Symbol(),tf,"Quantum",qde,0,1)>0 && reverse==false) || (iCustom(Symbol(),tf,"Quantum",qde,1,1)>0 && reverse)) && buyclosed==false)
              {
               sellclosed=false;
            ...
            if(((iCustom(Symbol(),tf,"Quantum",qde,1,1)>0 && reverse==false) || (iCustom(Symbol(),tf,"Quantum",qde,0,1)>0 && reverse)) && sellclosed==false)
              {
               buyclosed=false;
               if(wait>0 && thewait<wait)
                 {
...
void CheckCloseTrades()
  {
   ...
   if((iCustom(Symbol(),tf,"Quantum",qdc,1,1)>0 && closeonsell==true && reverse==false) || (iCustom(Symbol(),tf,"Quantum",qdc,0,1)>0 && closeonsell==true && reverse) || (CloseTrades==true))
     {
      thewait=0;
      DeleteLines();
      if(fifo==false)
      ...
      cycles+=1;
      //GlobalVariableSet(Symbol()+"DailyCycles",cycles);
     }
   if((iCustom(Symbol(),tf,"Quantum",qdc,0,1)>0 && closeonbuy==true && reverse==false) || (iCustom(Symbol(),tf,"Quantum",qdc,1,1)>0 && closeonbuy==true && reverse) || (CloseTrades==true))
     {
      thewait=0;
      DeleteLines();
      if(fifo==false)
